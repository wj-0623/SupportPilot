...
| 威胁 | 主要控制 | 剩余风险与生产措施 |
| --- | --- | --- |
| 跨租户读取 | JWT tenant/customer claim；scope mapping；仓储级 tenant 条件；404 隐藏资源存在性 | PostgreSQL 建议再加 Row Level Security 和集成测试 |
...
| Webhook 伪造 | 独立 secret、原始 body HMAC、时间窗、事件 ID 唯一约束、只存 hash | secret 轮换期需支持双 secret |
